...
export const billingRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post<{ Params: { id: string } }>('/api/transactions/:id/verify', async (request, reply) => {
    try {
      ...
      if (!transaction) return reply.code(404).send({ error: 'Transaction not found' });
      if (transaction.status !== 'PENDING') return reply.code(400).send({ error: 'Transaction is not pending' });
      ...
      const io = getIO();
      io.to(`session:${transaction.sessionId}`).emit('sessionSynced', syncData);
      io.to(`restaurant:${transaction.session.table.restaurantId}`).emit('adminStateSynced');

      return reply.code(200).send({ success: true, transactionId: transaction.id });
    } catch (error) {
      fastify.log.error(error);
      return reply.code(500).send({ error: 'Failed to verify transaction' });
    }
  });
